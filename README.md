...
Plugins need to add the plugin name before the model name. For example:

    ~/foreman/config/hooks/katello/product/before_create/01_integrate_catalog.sh
    ~/foreman/config/hooks/foreman_tasks/task/dynflow_task/after_create/90_record_action.sh
...
Every object (or model in Rails terms) in Foreman or in a plugin can have hooks.
Check `~foreman/app/models` or the plugin for the full list, but these are the interesting ones:
...
A JSON representation of the hook object will be passed in on stdin. This json is
generated by the v2 API views. A utility to read this with jgrep is provided
in `examples/hook_functions.sh` and sourcing this utility script will be
enough for most users.  Otherwise, you may want to ensure stdin is closed.
